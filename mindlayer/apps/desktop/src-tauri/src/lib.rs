...
const KEYCHAIN_SERVICE: &str = "com.mindlayer.desktop";
...
fn check_safari_permissions() -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "message": match status {
            ...
            capture::PermissionStatus::NeedsFullDiskAccess => "Full Disk Access required. Go to System Settings > Privacy & Security > Full Disk Access and enable Mindlayer.",
...
fn set_api_key(key: String, state: tauri::State<AppState>) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let (subkey, _) = hkcu.create_subkey("SOFTWARE\\Mindlayer").map_err(|e| e.to_string())?;
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let app_dir = config_dir.join("mindlayer");
...
fn clear_api_key(state: tauri::State<AppState>) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        use winreg::RegKey;
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        if let Ok(subkey) = hkcu.open_subkey_with_flags("SOFTWARE\\Mindlayer", KEY_WRITE) {
    ...
    #[cfg(target_os = "linux")]
    {
        use std::fs;
        let config_dir = dirs::config_dir().ok_or("No config directory")?;
        let key_file = config_dir.join("mindlayer").join("api_key");
...
fn load_saved_api_key() -> Option<String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        use winreg::RegKey;
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        hkcu.open_subkey("SOFTWARE\\Mindlayer")
            .ok()
...
fn get_config_path() -> Option<std::path::PathBuf> {
    dirs::config_dir().map(|p| p.join("mindlayer"))
}
...
fn setup_tray(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let quit = MenuItemBuilder::with_id("quit", "Quit Mindlayer").build(app)?;
    ...
    let _tray = TrayIconBuilder::new()
        ...
        .menu(&menu)
        .tooltip("Mindlayer - Mapping enabled")
