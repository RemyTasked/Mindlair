/**
 * Mind Garden - Outlook Calendar Content Script
 * 
 * Injects the Mind Garden sidebar and floating widget into Outlook Calendar.
 */

console.log('🌱 Mind Garden loading for Outlook Calendar...');
...
function init() {
  // Wait for Outlook Calendar to fully load
  const checkCalendarLoaded = setInterval(() => {
    // Outlook has different selectors depending on version
    const mainContent = document.querySelector('[data-app-section="MainModule"]') || 
...
function injectMindGarden() {
  ...
  console.log('🌱 Mind Garden injected into Outlook Calendar');
}
...
async function fetchData() {
  ...
  try {
    // Get stress forecast
    stressForecast = await browserAPI.runtime.sendMessage({ type: 'GET_STRESS_FORECAST' });
    updateForecastUI(stressForecast);
    
    // Get today's meetings
    currentMeetings = await browserAPI.runtime.sendMessage({ type: 'GET_TODAYS_MEETINGS' });
    updateMeetingsUI();
    
    // Get flow count
    const { dailyFlowCount } = await browserAPI.storage.local.get('dailyFlowCount');
