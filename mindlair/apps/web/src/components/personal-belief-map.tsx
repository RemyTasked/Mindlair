...
export default function PersonalBeliefMap({
  ...
  // Track if device supports hover (fine pointer like mouse/trackpad)
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      // On narrow screens with selection, reduce map height to give more space to detail panel
      const baseReserve = useTimeline ? 260 : 160;
      const reserve = isNarrow && selected ? baseReserve + 40 : baseReserve;
      const maxH = isNarrow && selected ? 480 : 540;
      setDims({ w: r.width, h: Math.max(280, Math.min(r.height - reserve, maxH)) });
    };
    measure();
  ...
  const activity = useMemo(() => {
    ...
    const act = interpolateTimelineActivity(snapshots, timeValue, nodeLabelMap);
    
    // Roll up merged concept activity into survivor nodes
    for (const n of nodes) {
      const survivorAct = act.get(n.id);
      let totalCount = survivorAct?.positionCount ?? 0;
      let direction = survivorAct?.direction ?? n.direction;
      
      // Add activity from absorbed concepts
      if (n.mergedFrom && n.mergedFrom.length > 0) {
        for (const absorbedId of n.mergedFrom) {
          const absorbedAct = act.get(absorbedId);
          if (absorbedAct) {
            totalCount += absorbedAct.positionCount;
            // Use absorbed direction if survivor has no activity
            if (!survivorAct || survivorAct.positionCount === 0) {
              direction = absorbedAct.direction;
            }
          }
        }
      }
              ...
              {nodes.map((node, ni) => {
                ...
                return (
                  <g
                    ...
                    onMouseEnter={canHover ? () => setHoveredId(node.id) : undefined}
          ...
          {nodes.length > 0 && useTimeline && discoveryText && (
            <div
              ...
              style={{
                position: "absolute",
                bottom: 14,
                left: isNarrow && selected ? 12 : 20,
                right: isNarrow && selected ? 12 : selected ? 0 : 20,
                maxWidth: 420,
          ...
          >
            ...
            {isNarrow ? (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                ...
                >
                  {detailSummary != null ? detailSummary.sourceCount : "…"} sources
                  <span style={{ fontSize: 9, color: C.muted }}>{showSources ? "▲" : "▼"}</span>
                </button>
                <div
                  style={{
                    background: C.bg,
                ...
                >
                  {detailSummary != null ? detailSummary.reactionCount : "…"} reactions
                </div>
                <div
                  style={{
                    background: C.bg,
                ...
                {tensionRows && tensionRows.length > 0 && (
                  ...
                  >
                    Tension
                    <span style={{ fontSize: 9, color: C.muted }}>{showTension ? "▲" : "▼"}</span>
                  </button>
                )}
              </div>
            ) : (
            ...
            {!isNarrow && detailSummary?.engagementByMonth && detailSummary.engagementByMonth.length > 0 && (
              <div
                style={{
                  background: C.bg,
                  borderRadius: 8,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, order: isNarrow ? -1 : 0 }}>
              <div
                style={{
                  fontSize: 9,
                  color: getCatColor(selected.category),
              ...
              {!recentLoading &&
                recentPositions &&
                recentPositions.slice(0, isNarrow ? 2 : 5).map((row, i) => {
