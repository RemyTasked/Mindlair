...
export function mergeSmallConcepts(
  ...
  // Resolve id through mergedInto chain to canonical survivor (with cycle guard)
  ...
  // Rewire edges through merged nodes, dedupe parallel edges
  const edgeMap = new Map<string, MapEdge>();
  for (const edge of edges) {
    const src = resolve(edge.source);
    const tgt = resolve(edge.target);
    
    // Skip self-loops and edges to non-remaining nodes
    if (src === tgt) continue;
    if (!remainingNodeIds.has(src) || !remainingNodeIds.has(tgt)) continue;
    
    // Canonical key for deduplication (smaller id first)
    const lo = src < tgt ? src : tgt;
    const hi = src < tgt ? tgt : src;
    const key = `${lo}\0${hi}\0${edge.type}`;
    
    const existing = edgeMap.get(key);
    if (existing) {
      // Merge weights: sum for related, max for tension
      existing.weight = edge.type === 'related'
        ? existing.weight + edge.weight
        : Math.max(existing.weight, edge.weight);
    } else {
      edgeMap.set(key, {
        source: lo,
        target: hi,
